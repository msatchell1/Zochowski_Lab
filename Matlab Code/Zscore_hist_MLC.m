function [BB1_diffs, BB2_diffs] = Zscore_hist_MLC(Zscore_mats, i_fig_dir)
...
% num_phases = length(Zscore_mats)/2;
% 
% for phase = 1:num_phases
%     subplot(2,num_phases,phase);
%     histogram(BB1_diffs{phase})
% end

subplot(2,3,1);
histogram(BB1_diffs{1}, Facecolor=[0,0,1]); % NREMtest - pretest for BB1
title('IGNORE ALL PLOTS');
...
sgtitle('IGNORE PLOTS - NEEDS FIXING'); % Title for whole figure
...
sgtitle('IGNORE PLOTS - NEEDS FIXING');
