...
%%
%this gets your file to format [n1 n1 n1 n2 n2 n2; t11 t12 t13 t21 t22 t23]
%which is needed foe the code
clear

directory = 'C:\Users\satchelm\OneDrive - Umich\Desktop\Zochowski Lab\Saved Figures Since Fixing STDP\Saved Figures 6-22-22\vary Idrive_LE, LEtoBB_plas_mult = 0.3\Idrive_LE = 1.5\';
filenames = {'pretest_BB1_LE_spikes', 'pretest_BB2_LE_spikes', 'NREMtest_BB1_LE_spikes', 'NREMtest_BB2_LE_spikes',  ...
     'posttest_BB1_LE_spikes', 'posttest_BB2_LE_spikes'};
 
num_files = length(filenames);
Zscore_mats = cell(num_files, 1); % List for holding the Zscore matrices  
data_in_cell={};
for i = 1:num_files % Loops through each file, creating and saving Zscore figures
    file = filenames{i};
    ...
    michaels_spikes_cell = importdata(strcat(directory,file,'.txt'));
...
%     saveas(figure(4),strcat(directory,'AMD_',phase,'.png'));
    ...
    xlabel("Neuron ID")
    ylabel("Neuron ID")
    saveas(figure(5),strcat(directory,'Zscore_',phase,'.png'));
...
mat_indices = [1, 3, 5]; % Indicies for looping matrices in pairs
...
zipped_dotps = [elem_dotps(:)/max(elem_dotps), norm_elem_dotps(:)/max(norm_elem_dotps)] ; % combines into matrix for bar plotting and
% normalizes by max length so they can be plotted on same plot.
figure(6)
%bp = bar(zipped_dotps);
bp = bar(norm_elem_dotps);
xticks([1,2,3]);
xticklabels({'pre-test','REM-test','NREM/REM-test'});
title('Zscore Matrix Dot Products')
%xlabel('Phase')
ylabel('Normalized Dot Products')
%ylim([-0.02,0.15]);
%legend({'Unnormalized','Normalized'});
%legend('Location','best')
saveas(figure(6),strcat(directory,'Zscore Matrices Dotp.png'));
saveas(figure(6),strcat(directory,'Zscore Matrices Dotp.pdf'));



Zscore_hist(Zscore_mats, directory) % Plots change in Zscore histograms.
